package com.example.swapi.presentation.vehicles
...
fun VehiclesTabContent(
    ...
    val state: VehiclesUiState by viewModel.uiState.collectAsState()
    val filtered = state.vehicles.filter { it.name.contains(state.searchQuery, ignoreCase = true) }
    ...
    when {
        state.isLoading && state.vehicles.isEmpty() -> LoadingStateView(Modifier.padding(padding))
            ...
            LazyColumn(
                ...
                items(filtered, key = { it.id }) { item ->
                    CatalogItemCard(
                        ...
                        onClick = { onVehicleClick(item.id) }
                    )
                }
            }
...
fun VehicleDetailsScreen(
    viewModel: VehiclesDetailsViewModel,
    onBack: () -> Unit
) {
    val state by viewModel.uiState.collectAsState()
    val item = state.item

    AppScreenScaffold(title = item?.name ?: AppStrings.VehiclesTab, onBack = onBack, centerTitle = true) { padding ->
        when {
            ...
            item == null -> MessageStateView(AppStrings.NoResultsFound, Modifier.padding(padding))
                ...
                item {
                    DetailHeroCard(
                        title = item.name,
                        subtitle = "${AppStrings.ModelPrefix}: ${item.model}  ·  ${AppStrings.VehicleClassPrefix}: ${item.vehicleClass}"
                    )
                }
                ...
                item { DetailInfoCard(AppStrings.VehicleClassPrefix, item.vehicleClass) }
