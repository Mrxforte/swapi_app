package com.example.swapi.presentation.species
...
@Composable
fun SpeciesTabContent(
    viewModel: SpeciesViewModel,
    onSpeciesClick: (String) -> Unit,
    padding: PaddingValues
) {
    val state: SpeciesUiState by viewModel.uiState.collectAsState()
    val filtered = state.speciesList.filter { it.name.contains(state.searchQuery, ignoreCase = true) }
    ...
    when {
        state.isLoading && state.speciesList.isEmpty() -> LoadingStateView(Modifier.padding(padding))
        state.errorMessage != null -> MessageStateView(state.errorMessage!!, Modifier.padding(padding), Dimens.ScreenPaddingLarge)
            ...
            LazyColumn(
                ...
                items(filtered, key = { it.id }) { item ->
                    CatalogItemCard(
                        title = item.name,
                        subtitle = AppStrings.withValue(AppStrings.ClassificationPrefix, item.classification),
...
fun SpeciesDetailsScreen(
    ...
    AppScreenScaffold(title = item?.name ?: AppStrings.SpeciesTab, onBack = onBack, centerTitle = true) { padding ->
        when {
            ...
            item == null -> MessageStateView(AppStrings.NoResultsFound, Modifier.padding(padding))
                ...
                item {
                    DetailHeroCard(
                        title = item.name,
                        subtitle = "${AppStrings.ClassificationPrefix}: ${item.classification}  ·  ${AppStrings.LanguagePrefix}: ${item.language}"
                    )
                }
                item { DetailInfoCard(AppStrings.NamePrefix, item.name) }
                item { DetailInfoCard(AppStrings.ClassificationPrefix, item.classification) }
                item { DetailInfoCard(AppStrings.DesignationPrefix, item.designation) }
                item { DetailInfoCard(AppStrings.AverageHeightPrefix, item.averageHeight) }
                item { DetailInfoCard(AppStrings.SkinColorsPrefix, item.skinColors) }
                item { DetailInfoCard(AppStrings.HairColorsPrefix, item.hairColors) }
                item { DetailInfoCard(AppStrings.EyeColorsPrefix, item.eyeColors) }
                item { DetailInfoCard(AppStrings.AverageLifespanPrefix, item.averageLifespan) }
                item { DetailInfoCard(AppStrings.LanguagePrefix, item.language) }
            }
        }
    }
}
